...
Formation phases wait for **all** participants; they never start a dance while some are still gathering. On timeout or an unreachable route the director safely resumes standard behavior. Motion durations and stagger values are seconds; duration is per participant, and the phase lasts through the last participant's offset. Motion playback uses one director clock, so fully synchronized clips do not drift. `facing` is `center`, `front`, or `keep`. Release blends to idle, then seeds each resident's normal controller from its current pose.
