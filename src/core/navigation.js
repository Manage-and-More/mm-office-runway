...
export function createNavigation() {
  ...
  function nearest(x, z) {
    ...
    for (let dy = -3; dy <= 3; dy++) for (let dx = -3; dx <= 3; dx++) {
      ...
      if (cx < 0 || cy < 0 || cx >= SIZE || cy >= SIZE || !grid[id]) continue;
      const p = point(id), d = Math.hypot(p.x - x, p.z - z);
      if (d < distance && segmentClear(x, z, p.x, p.z)) { distance = d; best = id; }
    }
    return best;
  }
  return {
    register(owner, definitions) {
      ...
      obstacles.set(owner, list); grid = null;
      return () => { if (obstacles.get(owner) === list) { obstacles.delete(owner); grid = null; } };
    },
    isWalkable, segmentClear,
    findPath(start, end) {
      if (!isWalkable(start.x, start.z) || !isWalkable(end.x, end.z)) return null;
      if (segmentClear(start.x, start.z, end.x, end.z)) return [{ x: end.x, z: end.z }];
      if (!grid) {
        grid = new Uint8Array(SIZE * SIZE);
        for (let i = 0; i < grid.length; i++) { const p = point(i); grid[i] = isWalkable(p.x, p.z) ? 1 : 0; }
      }
      const from = nearest(start.x, start.z), goal = nearest(end.x, end.z);
      ...
      while (head < tail && parent[goal] < 0) {
        ...
        for (const [dx, dz] of [[1, 0], [-1, 0], [0, 1], [0, -1]]) {
          ...
          if (x < 0 || z < 0 || x >= SIZE || z >= SIZE || !grid[next] || parent[next] >= 0) continue;
          // Neighbour clearance avoids clipping corners of thin SVG beds.
          const a = point(id), b = point(next);
          if (!segmentClear(a.x, a.z, b.x, b.z)) continue;
      ...
      while (index < route.length) {
        ...
        for (let i = index + 1; i < route.length; i++) {
          if (!segmentClear(current.x, current.z, route[i].x, route[i].z)) break;
          farthest = i;
        }
